...
public class player : MonoBehaviour
{
    ...
    public bool isGround 
    {
        get
        {
            if (transform.position.y < 0.061f) return true; //如果 Y 軸 小於 0.0X 傳回 true
    ...
    private void Move()
    {
        ...
        //就可以直接使用關鍵字 transform 取得此物件 Transfom元件
    ...
    private void HitProp(GameObject prop)
    {
        if (prop.tag == "長長的麵包")
        {
            aud.PlayOneShot(soundbread, 1);     //喇叭.撥放一次音效(音效片段, 音量)
            Destroy(prop);                      //刪除(物件)
        }
        else if (prop.tag == "熱狗")
        {
            aud.PlayOneShot(soundhotdog, 1);     
            Destroy(prop);
        }

        gm.GetProp(prop.tag);                   //告知 GM 取得道具(將道具標籤傳遞過去)
    }
    #endregion
    ...
    private void Start()
    {
        //GetComponnt<泛型>() 泛型方法 - 泛型 所有類型 Rigibody, Transform, Collider...
        //剛體 = 取得元件<剛體>();
